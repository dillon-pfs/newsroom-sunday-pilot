...
export function LiveScoreboard({ gameId }: { gameId?: string }) {
  ...
  const ageMs = data?.lastSuccessfulFetchAt && now !== null ? Math.max(0, now - Date.parse(data.lastSuccessfulFetchAt)) : null;
  const stale = failed || data?.availability === "stale" || (ageMs !== null && ageMs > (data?.staleAfterMs ?? 90_000));
    ...
    <section className="space-y-3" aria-label="NFL scoreboard" aria-busy={!data && !failed}>
      <div className="flex flex-wrap items-end justify-between gap-2">
        ...
        <p className={`font-mono text-[10px] uppercase ${stale ? "text-warn" : "text-ink-soft"}`}>
          {data ? `${stale ? "Updates delayed · Last checked" : "Checked"} ${ageLabel}` : failed ? "Scores temporarily unavailable" : "Loading scores…"}
        </p>
      </div>
      <p role="status" className="sr-only">{stale ? "Score updates are delayed." : data ? "Scoreboard available." : failed ? "Scores temporarily unavailable." : "Loading scoreboard."}</p>
